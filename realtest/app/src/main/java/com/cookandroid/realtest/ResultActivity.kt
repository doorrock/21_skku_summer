...
//            save_button.visibility = View.GONE
    ...
    private fun setBarChart(flag: Array<Boolean>,
        ...
        for(i in 0..7){
            if(flag[i]) {
                ...
                if(items_per[i] > category_per[i]) colorList.add(ContextCompat.getColor(this@ResultActivity, R.color.chart_red))
                else colorList.add(ContextCompat.getColor(this@ResultActivity, R.color.main_color))
            }
        }
        ...
        val d1 = BarDataSet(entries1, "DataSet")
            .apply{
                ...
                valueTextColor = ContextCompat.getColor(this@ResultActivity, R.color.black)
                valueTextSize = 14f
            }
        val d2 = BarDataSet(entries2, "DataSet")
            .apply{
                setDrawIcons(false)
                setDrawValues(true)
                color = ContextCompat.getColor(this@ResultActivity, R.color.chart_yellow)
                valueFormatter = PerFormatter()
                valueTextColor = ContextCompat.getColor(this@ResultActivity, R.color.black)
                valueTextSize = 14f
            }
    ...
    private fun initBarChart(labels: ArrayList<String>){
        barChart_main.run{
            ...
            legend.run{
                ...
                legenditem1.formColor = ContextCompat.getColor(this@ResultActivity, R.color.main_color)
                legenditem1.label = "제품 함량"
                val legenditem2 = LegendEntry()
                legenditem2.formColor = ContextCompat.getColor(this@ResultActivity, R.color.chart_red)
                legenditem2.label = "제품 함량 (주의)"
                val legenditem3 = LegendEntry()
                legenditem3.formColor = ContextCompat.getColor(this@ResultActivity, R.color.chart_yellow)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.compare -> {
                compare_dialog = View.inflate(this@ResultActivity, R.layout.dialog_history, null)
